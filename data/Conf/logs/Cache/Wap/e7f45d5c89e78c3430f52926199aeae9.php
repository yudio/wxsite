...
$(function(){
	$("#scratchpad").wScratchPad({
		...
		color : "#a9a9a7",
		size :12,
		scratchDown : function(e,i){
			if(text){
					text=false;
					document.getElementById('prize').innerHTML=winprize;
					document.getElementById('txt').innerHTML=winprize;
				}
			if(i>10&&e.pageX>30&&e.pageX<150&&e.pageY>10&&e.pageY<30){
				if(goon){
				if(zjl!=''||zjl!=0){
						$("#zjl").slideToggle(500);
		...
		scratchMove :function(e,i){
		if(i>10&&e.pageX>30&&e.pageX<150&&e.pageY>10&&e.pageY<35){
					if(goon){
					if(zjl!=''||zjl!=0){
							$("#zjl").slideToggle(500);
							//$("#outercont").slideUp(500)
