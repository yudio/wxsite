...
<section>
    ...
    <ul id="list_ul" class="list_ul">
        <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
            <a href="<?php if($vo['url'] == ''): ?>/npWap/Nicepa/lists.act?classid=<?php echo ($vo['id']); ?>&token=<?php echo ($vo['token']); else: echo ($vo["url"]); endif; ?>">
                <dl class="tbox">
                    <dd>
        ...
        <li>
            <a href="/npWap/Nicepa/lists.act?classid=0&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>">
                <dl class="tbox">
                    <dd>
