...
var DialogBox = (function () {
	return {
			...
			var n = 0,
				_showDom = function() {
					...
					if(window == parent.window) {
						...
						if(_t < _scrollTop) {	// 如果该值小于0，则设置为10，否者会顶到页面上端了
							_t = _scrollTop;
						}
					}else {
						var diff = 95;
						...
						if(_t == _scorllHeight) {	// 如果该值小于0，则设置为10，否者会顶到页面上端了
							_t += 10;
						}
		...
		popup : function(boxDom, shadeDom, titleBox, closeBtn, onClose, showFast) {
			...
			if(titleBox && !boxDom.onmousemove) {	// 制造移动效果
				$initDragItem({
					barDom : titleBox,
					targetDom : boxDom
				});
		...
		alert : function(option) {
			...
			if(typeof option == "string") {		// 如果只传一个string过来，则用默认样式展示
				opt.content = option;
			}else {
				for(var i in option) {
					opt[i] = option[i];
			...
			param.type = opt.type;
			param.cancelValue = null;	// 把取消按钮设置为空
		...
		/**
		 * 提示内容小纸条
		 * @param target	document	小纸条弹出参考对象
		...
		tips : function(target, content, dir, notFadeaway, width) {
			...
			if(!notFadeaway) {	// 如果设置不消失，则不进行消失的设置
				var n = 0;
		...
		showSendPreview : function(sendFn) {
			if(!$id("preview_by_weixin_box")) {
				var divHtml='<div class="ui-dialog" id="preview_by_weixin_box" style="display:none;width=320px">'+
						'<div class="js_drag ui-dialog__hd"><span class="js_poptitle">发送预览</span><i class="js_close ui-dialog__x"></i></div>'+
						'<div class="js_content ui-dialog__bd">'+
						'<form id="preview_by_weixin_form" action="#">'+
						'<input id="preview_by_weixin_input" class="txt_short" type="text" placeholder="手机号/QQ号"/>'+
						'<input id="preview_by_weixin_submit" type="submit" style="display:none;" />'+
						'</form></div>'+
				'<div class="ui-dialog__ft">'+
				'<a id="preview_by_weixin_confirm" class="js_submit ui_btn ui_btn_strong">发送</a>'+
