...
FCAPP.HOUSE.FULL3D = {
    ...
    init: function() {
        ...
        window.shareData.linkKeep = '/WebEstate/'+WXUID+'/index?wechatid='+WECHATID;
    ...
    go3D: function(url) {
        var t = new Date();
        //FCAPP.Common.jumpTo('/Webestate/Picfullshow'+url); //TODO
		FCAPP.Common.jumpTo('/WebEstate/'+WXUID+'/picfullshow',{sid:url});
    },
    ...
    back2List: function() {
        if (/Mac OS/i.test(navigator.userAgent)) {
            ...
            var url = 'http://localhost/WebEstate/'+WXUID+'/listHouse?eid='+EID + location.search.replace('&houseid=' + gQuery.houseid, '');
            location.href = url;
    ...
    loadData: function() {
        ...
        $.ajax({
            ...
            url:'/WebEstate/'+WXUID+'/picfulldata?hid='+HID,
			dataType: 'jsonp',
            error: function() {
                FCAPP.Common.msg(true, {
                    msg: '无效的户型！'
        ...
        if (data.rooms) {
            ...
            for (var i = 0, il = rooms.length; i < il; i++) {
                if (rooms[i].id == gQuery.houseid && rooms[i].full3d) {
                    ...
                    R.currPlace.html((rooms[i].pname || '') + '-' + rooms[i].name);
                    window.shareData.desc = '户型【360度全景】-' + rooms[i].pname + rooms[i].name;
                    window.shareData.descKeep = '户型【360度全景】-' + rooms[i].pname + rooms[i].name;
                    break;
                }
            }
            if (full3d.length > 0) {
