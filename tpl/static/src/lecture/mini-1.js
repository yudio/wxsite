var Swiper = function (f, b) {
            ...
            if (a.isMoved) {
                ...
                if (0 < a.positions.current) {
                    ...
                    if (a.positions.current < -f) {
                        ...
                        if (b.freeMode) {
                            if (b.freeModeFluid) {
                                var e = 1000 * b.momentumRatio, d = a.positions.current + a.velocity * e, g = !1, F, m = 20 * Math.abs(a.velocity) * b.momentumBounceRatio;
    ...
    function I(c, d, e) {
        ...
        if (a.support.transitions || !b.DOMAnimation) {
            ...
            var g = k ? a.getWrapperTranslate("x") : a.getWrapperTranslate("y"), h = "to" == d && 0 <= e.speed ? e.speed : b.speed, m = Math.ceil((c - g) / h * (1000 / 60)), l = g > c ? "toNext" : "toPrev", p = "toNext" == l ? g > c : g < c;
            if (a._DOMAnimating) {
                return;
            }
            f();
...
var myScroll;
function loaded() {
    myScroll = new iScroll("wrapper", {checkDOMChanges: true, hScrollbar: false, hScroll: false, useTransform: false, onBeforeScrollStart: function (e) {
        var target = e.target;
        while (target.nodeType != 1) {
            target = target.parentNode;
        }
...
loaded();
$(".spinner").css({left: ($(window).width() - 32) / 2, top: ($(window).height() - 32) / 2});
var current = "";
var hasmove = false;
...
function hideSpinner() {
    ...
    mySwiper = new Swiper(".swiper-container", {pagination: ".pagination", paginationClickable: true, slidesPerView: "auto"});
...
(function () {
    var onBridgeReady = function () {
        WeixinJSBridge.on("menu:share:appmessage", function (argv) {
            WeixinJSBridge.invoke("sendAppMessage", {"appid": dataForWeixin.appId, "img_url": dataForWeixin.MsgImg, "img_width": "120", "img_height": "120", "link": dataForWeixin.url, "desc": dataForWeixin.desc, "title": dataForWeixin.title}, function (res) {
                (dataForWeixin.callback)();
            });
        });
        WeixinJSBridge.on("menu:share:timeline", function (argv) {
            (dataForWeixin.callback)();
            WeixinJSBridge.invoke("shareTimeline", {"img_url": dataForWeixin.MsgImg, "img_width": "120", "img_height": "120", "link": dataForWeixin.url, "desc": dataForWeixin.desc, "title": dataForWeixin.title}, function (res) {
                (dataForWeixin.callback)();
            });
        });
        WeixinJSBridge.on("menu:share:weibo", function (argv) {
            WeixinJSBridge.invoke("shareWeibo", {"content": dataForWeixin.weibodesc + " " + dataForWeixin.url, "url": dataForWeixin.url}, function (res) {
                (dataForWeixin.callback)();
            });
        });
        WeixinJSBridge.on("menu:share:facebook", function (argv) {
            WeixinJSBridge.invoke("shareFB", {"img_url": dataForWeixin.MsgImg, "img_width": "120", "img_height": "120", "link": dataForWeixin.url, "desc": dataForWeixin.desc, "title": dataForWeixin.title}, function (res) {
            });
        });
    };
